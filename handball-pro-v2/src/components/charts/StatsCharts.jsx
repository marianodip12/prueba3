...
const tooltipStyle = {
  contentStyle: { background: T.card, border: `1px solid ${T.border}`, color: T.text, fontSize: 11 },
...
export const HBarChart = ({ data, dataKeys, height = 260 }) => (
  <ResponsiveContainer width="100%" height={height}>
    <BarChart data={data} layout="vertical" margin={{ left: 0, right: 10 }}>
      ...
      {dataKeys.map((dk, i) => (
        <Bar key={dk.key} dataKey={dk.key} name={dk.name} fill={dk.color || CHART_COLORS[i]}
          stackId={dk.stack} radius={!dk.stack ? [0, 4, 4, 0] : undefined} />
      ))}
    </BarChart>
  </ResponsiveContainer>
)
...
export const VBarChart = ({ data, dataKeys, height = 240 }) => (
  <ResponsiveContainer width="100%" height={height}>
    <BarChart data={data}>
      ...
      {dataKeys.map((dk, i) => (
        <Bar key={dk.key} dataKey={dk.key} name={dk.name} fill={dk.color || CHART_COLORS[i]}
          stackId={dk.stack} radius={!dk.stack ? [4, 4, 0, 0] : undefined}>
  ...
  <ResponsiveContainer width="100%" height={height}>
    <LineChart data={data}>
      ...
      <XAxis dataKey="min" stroke={T.muted} tick={{ fontSize: 10, fill: T.muted }}
        label={{ value: 'min', position: 'insideRight', offset: -5, fill: T.muted, fontSize: 10 }} />
      <YAxis stroke={T.muted} tick={{ fontSize: 10, fill: T.muted }} />
      <Tooltip {...tooltipStyle} />
      <Legend wrapperStyle={{ fontSize: 11 }} />
      {dataKeys.map((dk, i) => (
        <Line key={dk.key} type="monotone" dataKey={dk.key} name={dk.name}
          stroke={dk.color || CHART_COLORS[i]} strokeWidth={2} dot={false}
          strokeDasharray={dk.dashed ? '4 2' : undefined} />
      ))}
    </LineChart>
  </ResponsiveContainer>
)
  ...
  <ResponsiveContainer width="100%" height={height}>
    <PieChart>
      <Pie data={data.filter(d => d.value > 0)} cx="50%" cy="50%"
        innerRadius={50} outerRadius={90} dataKey="value" nameKey="name"
