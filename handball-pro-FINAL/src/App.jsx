...
const AppContent = () => {
  ...
  if (error) return (
    <div style={{background:T.bg,minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:12,padding:32}}>
      ...
      <button onClick={()=>window.location.reload()} style={{marginTop:12,background:T.accent,color:T.bg,border:'none',borderRadius:8,padding:'10px 24px',fontFamily:T.font,fontWeight:'bold',cursor:'pointer',fontSize:13}}>🔄 Reintentar</button>
    </div>
  )
...
export default function App() {
  return <MatchProvider><AppContent/></MatchProvider>
}
